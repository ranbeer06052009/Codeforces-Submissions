...
#define ls(u) (u << 1)
#define rs(u) (u << 1 | 1)
#define repi(i, a, b, c) for (ll i = a; i <= b; i+=c)
#define rep(i, a, b) for (ll i = a; i <= b; i++)
#define rrepi(i, a, b, c) for (ll i = a; i >= b; i-=c)
...
void solve(){
    ...
    rep(i,1,n){
        ...
        rep(j,1,m){
            if(s[i-1][j-1]=='1'){
                //cur+=1;
                pref[i][j]=pref[i-1][j]+1+pref[i][j-1]-pref[i-1][j-1];
                nw+=a[i][j];
            }else{
    ...
    rep(i,1,n-k+1){
        rep(j,1,m-k+1){
            ll d=pref[i+k-1][j+k-1]-pref[i+k-1][j-1]-pref[i-1][j+k-1]+pref[i-1][j-1];
            d=abs(k*k-2*d);
            g=gcd(g,d);
        }
    }
