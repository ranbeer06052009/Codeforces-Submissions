...
void solve(){
    ...
    ll cur_j = 0;
    for (int l = 1; l <= k; ++l) {
        ll target_rem = (1 - l) % a[l - 1];
        if (target_rem < 0) target_rem += a[l - 1];
        ll x, y;
        ll g = extGCD(cur_L, a[l - 1], x, y);
        ll diff = target_rem - cur_j;
        if (diff % g != 0) {
            cout << "NO
";
            return;
        ...
        ll rem1 = x % mod;
        ll rem2 = (diff / g) % mod;
        ll cur_x = mulmod(rem1, rem2, mod);
        if (cur_x < 0) cur_x += mod;
        if (cur_x > 0) {
            if (cur_L > m / cur_x) {
                cout << "NO
";
