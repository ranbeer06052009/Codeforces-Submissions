...
void solve(){
    ...
    for(int i=0; i<n; i++){
        if(i%2==0){
            if(a[i]=='1')ae1.push_back(i);
            if(b[i]=='1')be1.push_back(i);
        }else{
            if(a[i]=='1')ao1.push_back(i);
            if(b[i]=='1')bo1.push_back(i);
        }
    }
    if(ae1.size()!=be1.size()||ao1.size()!=bo1.size()){
        cout<<-1;
        return;
    }
    ll cnt=0;
